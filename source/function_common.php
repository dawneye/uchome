...
function clearcookie() {
	...
	ssetcookie('cloud_conuin', '', -86400 * 365);
	ssetcookie('cloud_conopenid', '', -86400 * 365);
...
function qqconnect() {
	global $_SGLOBAL, $_SC, $_SCONFIG;
	if(!$_SCONFIG['cloud_open'] || !$_SCONFIG['cloud_site_id'] || !$_SCONFIG['cloud_site_key']) {
		$_SGLOBAL['connect'] = 0;
	} else {
		$_SGLOBAL['connect'] = 1;
	}
	...
	if($_SGLOBAL['connect']) {
		include_once(S_ROOT.'./source/class_connect.php');
		include_once(S_ROOT.'./source/function_connect.php');
		cloud_token();
		if(empty($_SGLOBAL['qqconnect'])) {
			$_SGLOBAL['qqconnect'] = new Cloud_Connect($_SCONFIG['cloud_site_id'], $_SCONFIG['cloud_site_key']);
		}
	}
