...
function delAttach(id) {
	try {
		...
		$('attachbody').removeChild($('attach_' + id).parentNode);
	}
...
function insertAttach(id) {
	...
	if(is_ie || userAgent.indexOf('firefox') >= 1) {
		...
		inhtml += '<img src="' + picPath +'" width="60" height="80" /><br/>';
	}
	...
	inhtml += localfile + '&nbsp;<span id="showmsg' + id + '"><a class="btn" href="javascript:;" onclick="delAttach(' + id + ')">删除</a></span>';
	inhtml += '<br/>图片描述：<textarea class="span6" name="pic_title" cols="40" rows="2"></textarea>';
